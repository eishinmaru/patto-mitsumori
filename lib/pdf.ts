...
export async function downloadEstimatePDF(
  ...
  txt(clientName, colLeft, ly, { size: 13, bold: true, color: "#1b3a6b" });
  txt(" Onchu", colLeft + doc.getTextWidth(clientName) * (13 / 10) * 0.352, ly, {
    size: 10,
    color: "#1b3a6b",
  });
  ly += 1;
  ...
  txt("Generated by Patto-Mitsumori", pageW / 2, pageH - 6, {
    size: 7,
    color: "#aaaaaa",
    align: "center",
  });
  ...
  // 8. 保存
  // ══════════════════════════════════════════════════════════════
  const filename = `mitsumori_${estimate.clientName || "untitled"}_${estimate.issueDate}.pdf`;
  doc.save(filename);
}